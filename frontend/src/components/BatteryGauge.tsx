...
export function BatteryGauge({ soc, powerKw, capacityKwh = 400 }: BatteryGaugeProps) {
  ...
  const getSocColor = (soc: number) => {
    ...
    if (soc <= 50) return { text: "text-amber-400", bg: "bg-amber-500", border: "border-amber-500/60" };
    return { text: "text-emerald-400", bg: "bg-emerald-500", border: "border-emerald-500/60" };
  };
    ...
    <div className="scada-panel rounded-none p-4 font-mono flex flex-col justify-between h-full">
      <div>
        <div className="flex items-center justify-between mb-3 pb-2 border-b border-slate-800">
          ...
          <span className={`text-[10px] font-bold font-mono px-2 py-0.5 border ${colors.border} ${colors.text} bg-slate-900`}>
            {soc}% Charged
          </span>
        </div>
      ...
      <div className="space-y-1.5 rounded-none border border-slate-800 bg-slate-900 p-2.5 text-xs font-mono">
        ...
        <div className="flex items-center justify-between text-slate-300">
          ...
          <span className="text-emerald-400">98.4% (Optimal)</span>
        </div>
      </div>
    </div>
  );
