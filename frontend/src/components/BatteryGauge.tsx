...
export function BatteryGauge({ soc, powerKw, capacityKwh = 400 }: BatteryGaugeProps) {
  ...
  return (
    <div className="scada-panel p-4 font-mono flex flex-col justify-between h-full">
      <div>
        <div className="flex items-center justify-between mb-3 pb-2 border-b border-slate-700">
          ...
          <span className={`text-[10px] font-bold font-mono px-2 py-0.5 border ${colors.border} ${colors.text} bg-slate-950`}>
            {soc}% Charged
          </span>
        </div>
        ...
        <div className="relative my-3 flex items-center justify-center">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center font-mono">
            ...
            <span className="text-[10px] text-slate-400 mt-0.5">{storedEnergyKwh} / {capacityKwh} kWh</span>
          </div>
        </div>
      </div>

      {/* Bottom Power Flow Details */}
      <div className="space-y-1.5 border border-slate-700 bg-slate-950 p-2.5 text-xs font-mono">
        <div className="flex items-center justify-between text-slate-300">
          <span className="text-slate-400 font-sans">Current Mode</span>
        ...
        <div className="flex items-center justify-between text-slate-300">
          <span className="text-slate-400 font-sans">Health (SoH)</span>
          <span className="text-emerald-500 font-bold">98.4% (Optimal)</span>
        </div>
      </div>
    </div>
  );
